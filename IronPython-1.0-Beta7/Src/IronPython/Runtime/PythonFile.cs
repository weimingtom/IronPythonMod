...
namespace IronPython.Runtime {
    ...
    public class PythonFile : IDynamicObject, IDisposable {
        ...
        [PythonName("__new__")]
        public static PythonFile Make(ICallerContext context, object cls, string filename) {
            return Make(context, cls, filename, "r", -1);
        }

        [PythonName("__new__")]
        public static PythonFile Make(ICallerContext context, object cls, string filename, string mode) {
            return Make(context, cls, filename, mode, -1);
        }

        [PythonName("__new__")]
        public static PythonFile Make(ICallerContext context, object cls, string filename, string mode, int bufsize) {
            ...
            return new PythonFile(stream, context.SystemState.DefaultEncoding, filename, inMode);
        }

        [PythonName("__new__")]
        public static PythonFile Make(ICallerContext context, object cls, Stream stream) {
            string mode;
            if (stream.CanRead && stream.CanWrite) mode = "w+";
            else if (stream.CanWrite) mode = "w";
            else mode = "r";

            return new PythonFile(stream, context.SystemState.DefaultEncoding, mode);
        }

        [PythonName("__new__")]
        public static PythonFile Make(ICallerContext context, object cls, Stream stream, string mode) {
            return new PythonFile(stream, context.SystemState.DefaultEncoding, mode);
        }
