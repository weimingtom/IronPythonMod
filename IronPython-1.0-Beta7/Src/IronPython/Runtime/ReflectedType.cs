...
namespace IronPython.Runtime {
    ...
    public partial class ReflectedType : PythonType, IFancyCallable, IMapping, IContextAwareMember, ICallableWithCallerContext {
        ...
        object IronPython.Runtime.IFancyCallable.Call(object[] args, string[] names) {
            Initialize();

            if (ctor == null) throw Ops.TypeError("cannot create an instance of {0}", this);
            IFancyCallable ifc = ctor as IFancyCallable;
            if (ifc != null) {
                object newObject = ifc.Call(PrependThis(args), names);
                if (newObject == null) return null;
                ...
                object initFunc;
                if (this.TryGetAttr(DefaultContext.Default, newObject, SymbolTable.Init, out initFunc) && !(initFunc is InitMethod)) {
        ...
        public object Call(object[] args, string[] names) {
            ...
            for (int i = 0; i < names.Length; i++) {
                if (instance == null) {
                    ...
                    Ops.SetAttr(DefaultContext.Default, args[0], SymbolTable.StringToId(names[i]), args[args.Length - names.Length + i]);
                } else {
                    Ops.SetAttr(DefaultContext.Default, instance, SymbolTable.StringToId(names[i]), args[args.Length - names.Length + i]);
                }
            }
            return null;
        }
