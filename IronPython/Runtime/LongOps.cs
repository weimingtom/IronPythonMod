...
namespace IronPython.Runtime {
    ...
    public static partial class LongOps {
        ...
        public static object Make(PythonType cls, object o) {
            if (cls == LongType) {
                if (o is string) return ParseBigIntegerSign((string)o, 10);
                if (o is IronMath.BigInteger) return (IronMath.BigInteger)o;
                else if (o is int) return (long)(int)o;
                else if (o is double) return IronMath.BigInteger.Create((double)o);
                else if (o is long) return o;
                else {
                    Conversion conv;
                    BigInteger intVal;
                    intVal = Converter.TryConvertToBigInteger(o, out conv);
                    if (conv < Conversion.Truncation) return intVal;
                }
            } else {
                BigInteger intVal = null;
                if (o is string) intVal = ParseBigIntegerSign((string)o, 10);
                else if (o is IronMath.BigInteger) intVal = (IronMath.BigInteger)o;
                else if (o is int) intVal = (long)(int)o;
                else if (o is double) intVal = IronMath.BigInteger.Create((double)o);
                else if (o is long) intVal = (long)o;
                else {
                    Conversion conv;
                    intVal = Converter.TryConvertToBigInteger(o, out conv);
            ...
            if (o is Complex64) throw Ops.TypeError("can't convert complex to long; use long(abs(z))");
