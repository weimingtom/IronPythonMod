...
namespace IronPython.Runtime {
        ...
        internal static object FastNew(object context, object x) {
            ...
            if (x is string) {
                ...
                for (int i = 0; i < s.Length; i++) {
                    if (s[i] > '\x80') return StringOps.Make((ICallerContext)context, (PythonType)Ops.GetDynamicTypeFromType(typeof(String)), s, null);
                }
                return s;
            }
    ...
    /// </summary>
    public class ExtensibleString : ICodeFormattable, IRichComparable {
        private string self;
    ...
    public static class StringOps {
        ...
        [PythonName("__new__")]
        public static object Make(ICallerContext context, PythonType cls, object x) {
            if (cls == TypeCache.String) {
                return StringDynamicType.FastNew(context, x);
            } else {
        ...
        [PythonName("__new__")]
        public static object Make(ICallerContext context, PythonType cls, string s, string encoding) {
            if (cls == TypeCache.String) {
                return Decode(context, s, encoding);
            } else {
                return cls.ctor.Call(cls, s, encoding);
            }
        }
        #endregion
        ...
        public static string Decode(ICallerContext context, string s) {
            return Decode(context, s, null);
        }

        [PythonName("decode")]
        public static string Decode(ICallerContext context, string s, string encoding) {
            return RawDecode(context.SystemState, s, encoding);
        }

        [PythonName("encode")]
        public static string Encode(ICallerContext context, string s, string encoding) {
            return RawEncode(context.SystemState, s, encoding);
        }
        ...
        private static string RawDecode(SystemState state, string s, string encoding) {
            if (encoding == "raw-unicode-escape") {
                return LiteralParser.ParseString(s, true, true);
            }

            Encoding e;
            if (TryGetEncoding(state, encoding, out e)) {
                // CLR's encoder exceptions have a 1-1 mapping w/ Python's encoder exceptions
                // so we just clone the encoding & set the fallback to throw
                e = (Encoding)e.Clone();
        ...
        private static string RawEncode(SystemState state, string s, string encoding) {
            if (encoding == "raw-unicode-escape") {
                bool fUnicode = false;
