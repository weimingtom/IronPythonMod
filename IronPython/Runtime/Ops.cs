...
namespace IronPython.Runtime {
    ...
    public static partial class Ops {
        ...
        public static object Plus(object o) {
            ...
            throw Ops.TypeError("Bad operand for unary +");
        }
        ...
        public static object CallWithArgsTupleAndKeywordDictAndContext(ICallerContext context, object func, object[] args, string[] names, object argsTuple, object kwDict) {
            ...
            if ((kws == null || kws.Count == 0) && names.Length == 0) {
                ...
                if (argsTuple != null && args.Length == names.Length) {
                    largs = new List<object>((Tuple)argsTuple);
                    largs.AddRange(args);
                } else {
                ...
                return ic.Call(largs.ToArray(), lnames.ToArray());
            }
        }
        ...
        public static object CallWithArgsTupleAndKeywordDict(object func, object[] args, string[] names, object argsTuple, object kwDict) {
            IDictionary<object, object> kws = kwDict as IDictionary<object, object>;
            if (kws == null) throw Ops.TypeError("argument after ** must be a dictionary");

            if (kws.Count == 0 && names.Length == 0) {
                ArrayList largs = new ArrayList(args);
                if (argsTuple != null) {
                    largs.AddRange(Ops.GetCollection(argsTuple));
                }
                return Call(func, (object[])largs.ToArray(typeof(object)));
            } else {
                IFancyCallable ic = func as IFancyCallable;
                if (ic == null) throw new Exception("this object is not callable with keyword parameters");

                //!!! should use generics and look at fastpaths for common cases
                ArrayList largs = new ArrayList(args);
                ArrayList lnames = new ArrayList(names);

                if (argsTuple != null) {
                    largs.InsertRange(args.Length - names.Length, Ops.GetCollection(argsTuple));
                }


                foreach (KeyValuePair<object, object> entry in kws) {
                    lnames.Add(entry.Key);
                    largs.Add(entry.Value);
                }

                return ic.Call(largs.ToArray(), (string[])lnames.ToArray(typeof(string)));
            }
        }
